...
def plot_catchment(
    fdir, catchment_poly, result_polygon, lat, lng, lat_snap, lng_snap, wid, dirmap
):
    """
    ...
    plt.imshow(
        fdir, extent=fdir.extent, cmap="viridis", zorder=0, norm=LogNorm(), alpha=0.6
    )
...
def plot_polys(polygons: list, wid: str):
    ...
    plt.title(
        f"Subdivided downstream unit catchment for watershed id = '{wid}'"
